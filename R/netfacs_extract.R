...
netfacs.extract <- function(netfacs.data, level = 1, min.count = 1, min.prob = 0, min.specificity = 0, significance = 0.01){
  ...
  if(is.null(netfacs.data$used.parameters$test.condition)){
    ...
    net.data$probability.increase = net.data$observed.probability/net.data$expected.probability
  }
  net.data = net.data[net.data$observed.probability >= min.prob & net.data$count >= min.count & net.data$specificity >= min.specificity & net.data$pvalue<=significance,]
